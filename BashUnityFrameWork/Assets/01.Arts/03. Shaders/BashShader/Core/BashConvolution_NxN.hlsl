// BashConvolution_NxN.hlsl — AutoN + 정규화 모드 + signed 디코드 토글

inline float2 Bash_ClampUV(float2 uv, float2 texel)
{
    return clamp(uv, texel * 0.5, 1.0 - texel * 0.5);
}

// normalizeMode: 0=None, 1=Sum, 2=SumAbs
void Bash_ConvolutionKernelTex_Core_AutoN(
    ...
    out float4 OutColor)
{
    ...
    for (int j = -halfK; j <= halfK; j++)
    {
        ...
        {
            float2 suv = Bash_ClampUV(UV + TexelSize * float2(i, j), TexelSize);
...
// Use Pragmas = ON
void Bash_ConvolutionKernelTex_float(
    Texture2D SourceTex, SamplerState SourceSamp,
    float2 UV, float2 TexelSize,
    Texture2D KernelTex, SamplerState KernelSamp,
    float Scale, float Bias,
    ...
    out float4 OutColor)
{
    Bash_ConvolutionKernelTex_Core_AutoN(
...
void Bash_ConvolutionKernelTex_half(
    Texture2D SourceTex, SamplerState SourceSamp,
